from fastapi import FastAPI, datastructures, Query
...
def analyzeUserTweets(userId):
    ...
    for tweet in tweets: 
        ...
        response.append(Tweet(tweet.id,tweet.created_at,tweet.full_text,dict))
...
def analyzeUserTweets(usersIds: List[str] = Query(None)):
    ...
    for userId in usersIds:
        ...
        for tweet in tweets: 
            tweet.full_text = simplifyTweetText(tweet.full_text)
            dict = mapReducer(tweet.full_text)
            userResposne.append(Tweet(tweet.id,tweet.created_at,tweet.full_text,dict))
